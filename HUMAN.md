...
| C-05 | **P1.2 build calls:** (a) ~~defer the Next.js framework/build~~ → **reversed in P1.4** (see C-06); (b) **defer `tenant_id`** to P5 (D-06); (c) **RLS enabled default-deny in `0001`** (policies in P1.3); (d) added verbs `pull_queue` (all roles) + `update_ticket` (planner); (e) P1.2 **transition-policy baseline** (worker targets = planned/claimed/in_progress/in_review/archived; `merged` = GitHub gate; human owns approved/changes_requested/rejected) — exact role×transition + loop-back wiring refined in P2. | ACTIVE | All overridable — say the word. `foundation/data-model.md` Open questions tracks the P2 refinements. |
| C-06 | **P1.4 — Next.js on the current major.** Wired the framework on **next@16 + react@19**, bumping off Knovo's inherited Next 14 (D-08) because 14 carries 14 high-severity advisories — not acceptable for a fresh prod launch. Mechanical cost: `/api/agent/*` `[id]` handlers moved to Next 15+ async `params`. D-08's stack (Next.js App Router + Supabase + Vercel) is unchanged; only the version pin moved. Residual: 2 **moderate** transitive `postcss` advisories inside Next's bundle (CSS-stringify XSS) — not exploitable (LoopRR processes no untrusted CSS), no upstream fix yet; clears when Next bumps postcss. | ACTIVE | Overridable — if you'd rather pin Next 14 (or a specific 15.x), say so and I'll adjust. |
